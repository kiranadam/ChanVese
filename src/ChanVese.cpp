...
vector<double> ChanVese :: Heavyside_avg(Mat& src, Mat& phi, double epsilon)
{
	...
	for(unsigned i=0; i<src.rows; i++)
	{
		...
		{
			double H_phi = 0.5*(1+(2.0/CV_PI)*atan2((double)phi.at<uchar>(i,j),epsilon));
			c1_num += (double)src.at<uchar>(i,j)*H_phi;
			c1_den += H_phi;
			c2_num += (double)src.at<uchar>(i,j)*(1-H_phi);
			c2_den += 1-H_phi;
		}
	}
...
Mat ChanVese :: delta_phi(Mat& phi)
{
	...
	for(unsigned i=0; i<phi.rows; i++)
	{
		...
		{
			del_phi.at<double>(i,j) = (1/CV_PI)*( h/( pow(h,2) + pow((double)phi.at<uchar>(i,j),2) ));
		}
	}
...
double ChanVese :: perimeter_length(Mat& phi, Mat& del_phi)
{
	...
	for(unsigned i=0; i<phi.rows; i++)
	{
		...
		for(unsigned j=0; j<phi.cols; j++)
		{
			L += del_phi.at<double>(i,j) * abs_nabla_phi.at<double>(i,j);
		}
	}
...
vector<double> ChanVese :: CV_coefficients(Mat& phi, Mat& del_phi, double L, unsigned i, unsigned j)
{
	...
	C[0] = 1/sqrt(pow((phi.at<double>(i+1,j)-phi.at<double>(i,j)),2) + pow((phi.at<double>(i,j+1)-phi.at<double>(i,j-1)),2)/4 + zero_nullifier);
	C[1] = 1/sqrt(pow((phi.at<double>(i,j)-phi.at<double>(i-1,j)),2) + pow((phi.at<double>(i-1,j+1)-phi.at<double>(i-1,j-1)),2)/4 + zero_nullifier);
	C[2] = 1/sqrt(pow((phi.at<double>(i+1,j)-phi.at<double>(i-1,j)),2)/4 + pow((phi.at<double>(i,j+1)-phi.at<double>(i,j)),2) + zero_nullifier);
	C[3] = 1/sqrt(pow((phi.at<double>(i+1,j-1)-phi.at<double>(i-1,j-1)),2)/4 + pow((phi.at<double>(i,j)-phi.at<double>(i,j-1)),2) + zero_nullifier); 

	double F_den = h + dt*del_phi.at<double>(i,j) * mu * (p*pow(L,p-1)) * (C[0]+C[1]+C[2]+C[3]);
	double F_num = 0.0;

	for(unsigned k=0; k<4; k++)
	{
		F_num = (dt * del_phi.at<double>(i,j) * mu *(p*pow(L,p-1)) * C[k]) / F_den;
		F.push_back(F_num);
	}
		...
		for(unsigned l=0; l<iteration; l++)
		{
			...
			for(unsigned i=1; i < src.rows-1; i++)
			{
				...
				for(unsigned j=1; j < src.cols-1; j++)
				{
					...
					double Pi_j = phi.at<double>(i,j) + dt * del_phi.at<double>(i,j)*(nu + lambda1*pow(((double)src.at<uchar>(i,j)-c[0]),2) + lambda2*pow(((double)src.at<uchar>(i,j)-c[1]),2) );  
					//Update phi		
					new_phi.at<double>(i,j) = F[0]*phi.at<double>(i+1,j) + F[1]*phi.at<double>(i-1,j) + F[2]*phi.at<double>(i,j+1) + F[3]*phi.at<double>(i,j-1) + F[4]*Pi_j;
				}
			}
			...
			for(unsigned i=0; i < src.rows; i++)
			{
				new_phi.at<double>(i,0) = new_phi.at<double>(i,1);
				new_phi.at<double>(i,src.cols-1) = new_phi.at<double>(i,src.cols-2);

			}

			for(unsigned j=0; j < src.cols; j++)
			{
				new_phi.at<double>(0,j) = new_phi.at<double>(1,j);
				new_phi.at<double>(src.rows-1,j) = new_phi.at<double>(src.rows-2,j);
 	...
 	Mat psi = phi.clone();
  	...
  	for(unsigned k = 0; k < iterations && flag == false; k++)
  	{
    		...
    		for(unsigned i = 1; i < phi.rows-1; i++)
    		{
     			...
     			{
        			a = ( phi.at<double>(i,j) - phi.at<double>(i-1,j) )/h;
        			b = ( phi.at<double>(i+1,j) - phi.at<double>(i,j) )/h;
        			c = ( phi.at<double>(i,j) - phi.at<double>(i,j-1) )/h;
        			d = ( phi.at<double>(i,j+1) - phi.at<double>(i,j) )/h;

        			if(phi.at<double>(i,j) > 0)
				{
          				G = sqrt(max(pow(max(a,0.0),2),pow(min(b,0.0),2)) + max(pow(max(c,0.0),2),pow(min(d,0.0),2))) - 1.0;
				}
        			else if(phi.at<double>(i,j) < 0)
				{
        			...
        			else
				{
        			...
        			x = (phi.at<double>(i,j) >= 0)?(1.0):(-1.0);

        			psi.at<double>(i,j) = psi.at<double>(i,j) - dt*x*G;
      			}
    		}
		...
		M = 0.0;
    		...
    		for (unsigned i = 0; i < phi.rows; i++)
    		{
      			for (unsigned j = 0; j < phi.cols; j++)
      			{
        			if(abs(old_psi.at<double>(i,j)) <= h)
        			{
          				M++;
          				Q += abs(old_psi.at<double>(i,j) - psi.at<double>(i,j));
        			}
      			}
    		}
